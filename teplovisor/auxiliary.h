...
/*	struct VideoSensorResolutionsData {
	...
	struct VideoSensorParameters {
		VideoSensorResolutionData res;
		VideoSensorSettingsData set;
	};
	...
	inline void SendVideoSensorParameters(const VideoSensorParameters& params) {
		Pkt<VideoSensorResolutionData> pkt_res;
		pkt_res.type = VideoSensorResolutionType;
		pkt_res.data = params.res;
		Comm::instance().transmit(Port, sizeof(pkt_res), (uint8_t*)&pkt_res);

		Pkt<VideoSensorSettingsData> pkt_set;
		pkt_set.type = VideoSensorSettingsType;
		pkt_set.data = params.set;
		Comm::instance().transmit(Port, sizeof(pkt_set), (uint8_t*)&pkt_set);
	}
